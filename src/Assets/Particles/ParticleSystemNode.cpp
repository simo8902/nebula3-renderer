...
bool ParticleSystemNode::Setup(const Node* node, Mesh* mesh) {
    ...
    if (mesh && !mesh->verts.empty() && !mesh->idx.empty()) {
        ...
        bool indexConvertible = true;
        for (uint32_t idx : mesh->idx) {
            if (idx > std::numeric_limits<uint16_t>::max()) {
                indexConvertible = false;
                break;
            }
            indices16.push_back(static_cast<uint16_t>(idx));
        }

        if (indexConvertible && !indices16.empty()) {
            uint32_t baseIndex = 0;
            uint32_t numIndices = static_cast<uint32_t>(indices16.size());
            if (node->primitive_group_idx >= 0 && node->primitive_group_idx < (int)mesh->groups.size()) {
                const auto& group = mesh->groups[node->primitive_group_idx];
                baseIndex = group.firstIndex();
                numIndices = group.indexCount();
            }
            if (baseIndex < indices16.size()) {
                const uint64_t requestedEnd = static_cast<uint64_t>(baseIndex) + static_cast<uint64_t>(numIndices);
                const uint32_t safeEnd = static_cast<uint32_t>(std::min<uint64_t>(requestedEnd, indices16.size()));
                if (safeEnd > baseIndex) {
                    emitterMesh.Setup(
                        vertexData,
                        sizeof(ObjVertex),
                    ...
                    meshEmitterBuilt = emitterMesh.IsValid();
                }
            }
        }
    }
