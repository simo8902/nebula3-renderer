...
MapData* MapLoader::load_map(const std::string& filepath) {
    ...
    if (!load_internal(r, data)) {
        std::cerr << "load_internal failed\n";
        delete data;
        return nullptr;
    }
    return data;
...
bool MapLoader::load_internal(NDEVcBinaryReader& r, MapData* data) {
    if (!parse_header(r)) { std::cerr << "header fail\n"; return false; }
    if (!load_map_info(r, data)) { std::cerr << "map info fail\n"; return false; }
    if (!load_string_table(r, data)) { std::cerr << "string table fail\n"; return false; }
    if (!load_sets(r)) { std::cerr << "sets fail\n"; return false; }
    if (!load_event_list(r, data)) { std::cerr << "event list fail\n"; return false; }
    if (!load_event_mapping(r, data)) { std::cerr << "event mapping fail\n"; return false; }
    if (!load_templates(r, data)) { std::cerr << "templates fail\n"; return false; }
    if (!load_instances(r, data)) { std::cerr << "instances fail\n"; return false; }
    if (!load_groups(r, data)) { std::cerr << "groups fail\n"; return false; }
    if (!load_nav_blockers(r)) { std::cerr << "nav blockers fail\n"; return false; }
