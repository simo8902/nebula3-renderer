...
void GLParticleRenderer::Setup(NDEVC::Graphics::IShaderManager* sharedShaderManager) {
    if (shaderManager && ownsShaderManager) {
    ...
    if (sharedShaderManager) {
        ...
        ownsShaderManager = false;
    } else {
        shaderManager = new NDEVC::Graphics::OpenGL::OpenGLShaderManager();
        ownsShaderManager = true;
        if (!shaderManager) {
...
void GLParticleRenderer::Discard() {
    ...
    if (cornerIBO) glDeleteBuffers(1, &cornerIBO);
    if (sampler) glDeleteSamplers(1, &sampler);

    vao = cornerVBO = particleVBO = cornerIBO = sampler = 0;
